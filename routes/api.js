...
router.get('/drakjokes', async (req, res, next) => {
  ...
  if(!Apikey) return res.json(loghandler.notparam)
  if(listkey.includes(Apikey)) {
    const meme = JSON.parse(fs.readFileSync(__path +'/data/darkjokes.json'));
    const Meme = meme[Math.floor(Math.random() * meme.length)];
    let hasil = Meme.meme;
    data = await fetch(hasil).then(v => v.buffer())
    await fs.writeFileSync(__path +'/tmp/drakjokes.mp4', data)
    res.sendFile(__path +'/tmp/drakjokes.mp4')
  } else {
    res.json(loghandler.invalidKey)
  }
});
                                ...
                                .then(data => {
                                    var urlnya = data.data.url,
                                        ...
                                        res.json({
                                            status : true,
                                            creator : `${creator}`,
                                            message : `Ada Yang Error Lapor Ke Owner ${creator}`,
                                            result:{
                                                url:urlnya,
